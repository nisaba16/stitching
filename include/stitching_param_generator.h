...
public:
  explicit StitchingParamGenerator(const std::vector<cv::Mat>& image_vector, bool use_calibration = true, const std::string& feature_method = "SIFT", bool use_feature_mask = false, const std::string& debug_folder = "results");
...
private:
  ...
  std::string warp_type = "spherical";  // spherical handles arbitrary camera tilt (downward-facing rig)
