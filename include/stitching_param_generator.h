...
public:
  explicit StitchingParamGenerator(const std::vector<cv::Mat>& image_vector, bool use_calibration = true, bool use_sift = true, bool use_feature_mask = false);
...
private:
  ...
  bool use_sift_;  // true = SIFT (slower, more accurate), false = ORB (faster, less accurate)
  ...
  std::string warp_type = "cylindrical";  // Best for planar scenes like sports fields - keeps lines straight
  int expos_comp_type = cv::detail::ExposureCompensator::GAIN;
  int expos_comp_nr_feeds = 1;
  int expos_comp_nr_filtering = 2;
  int expos_comp_block_size = 32;
  std::string seam_find_type = "dp_color";  // Dyna warp typemic programming for better seams
  int blend_type = cv::detail::Blender::MULTI_BAND;
  int timelapse_type = cv::detail::Timelapser::AS_IS;
  float blend_strength = 6;
