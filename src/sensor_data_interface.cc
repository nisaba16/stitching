...
void SensorDataInterface::InitVideoCapture() {
  ...
  for (const auto& file : video_files_) {
    ...
    // Capture FPS from first successfully opened video
    if (fps_ == 0) {
      fps_ = capture.get(cv::CAP_PROP_FPS);
      std::cout << "Detected input FPS: " << fps_ << std::endl;
    }
  }
...
double SensorDataInterface::getFPS() {
  return fps_;
}
...
void SensorDataInterface::rewind_videos() {
  for (size_t i = 0; i < num_img_; ++i) {
    if (video_capture_vector_[i].isOpened()) {
      video_capture_vector_[i].set(cv::CAP_PROP_POS_FRAMES, 0);
      video_finished_[i] = false;
    }
  }
}
